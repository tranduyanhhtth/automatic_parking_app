...
void HidKeyboardCardReader::processKey(int key, const QString &text)
{
    ...
    if (!text.isEmpty())
    {
        // CR10E thường gửi ký tự ASCII số/ chữ in HOA
...
void HidKeyboardCardReader::finalize()
{
    ...
    {
        if (now - m_lastEmitMs >= m_debounceMs)
        {
            emit debugLog(QStringLiteral("[HID] RFID complete '%1'").arg(m_buffer.trimmed()));
            emit rfidScanned(m_buffer.trimmed());
            m_lastEmitMs = now;
        }
        else
        {
            emit debugLog(QStringLiteral("[HID] Debounced swipe ignored '%1'").arg(m_buffer));
        }
    }
    m_buffer.clear();
}
