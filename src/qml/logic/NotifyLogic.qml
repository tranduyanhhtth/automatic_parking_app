...
Item {
    ...
    function classify(msg){ if(!msg) return 'info'; const m=msg.toLowerCase(); if(m.indexOf('lỗi')===0 || m.indexOf('error')===0 || m.indexOf('không thể')>=0) return 'error'; if(m.indexOf('cảnh báo')===0 || m.indexOf('warning')===0) return 'warn'; if(m.indexOf('đã ')===0 || m.indexOf('thành công')>=0 || m.indexOf('saved')>=0) return 'success'; return 'info'; }
    ...
    function push(message, severity){
        ...
        while(msgs.count>=maxMessages) msgs.remove(0)
        const id=_nextId++
        msgs.append({ id:id, text:message, sev:severity, ts:Date.now(), count:1 })
        // Timer fade
